...
GlycoNetworks is a command line tool.
```
Usage: GlycoNetworks <Spectra file path> <Glycan database file path> [Mass tolerance] [Deltas]
...
Example command for running GlycoNetworks on this test dataset:
```
GlycoNetworks "HF01_20171017_QXH_LXY_2017YFF0205400_GIgM_F1_R1.pd.mgf" "N-glycan 182 human no multiple fucose.txt" 0.015 Hex,HexNAc,Fuc,NeuAc,Hex-HexNAc
```
The console output shows the additional glycans found by GlycoNetworks:
```
...
GlycoNetworks also generates auxiliary files with more details:
